...
// General Retail Store's product catalog — plain name/unit/price, deliberately no cement/aggregate
// category or attributes (that's the Materials pack's shape, not this one). Reuses
// /api/admin/materials/products, which is already generic (serviceId-scoped, category is optional) —
// no behavioral difference for a retail product that simply never sends a category.
const blankForm = { name: '', unit: 'each', price: '' };
...
export default function RetailProductsPage() {
  ...
  if (!serviceId) {
    return (
      <div>
        <PageHeader title="Products" subtitle="Catalog for this service" />
        <Card><EmptyState title="Pick a service" subtitle="Choose the General Retail Store service from the switcher at the top of the page to manage its products." /></Card>
      </div>
    );
  }
