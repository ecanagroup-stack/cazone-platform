...
// category concept and no ATC allocation picker (that's specific to the Materials pack). Product
// listing reuses /api/admin/materials/products (already generic, serviceId-scoped — nothing
// materials-specific about a GET), but checkout posts to its own /api/admin/retail/orders so the
// resulting Order is tagged channel: 'retail', distinct from Materials' 'shop'/'atc'.
export default function RetailCounterPage() {
  ...
  const submitOrder = async (overrideCredit = false, otp = '') => {
    setSubmitting(true);
    try {
      const r = await fetch('/api/admin/retail/orders', {
