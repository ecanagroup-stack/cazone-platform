...
// old fuel app's summary-book combined into one report.
export const GET = withOrg(async (request) => {
  try {
    ...
    for (const m of moves) {
      const key = `${m.productId}|${m.reason}`;
      const bucket = buckets.get(key) || { product: m.product.name, unit: m.product.unit, reason: m.reason, qty: 0 };
      bucket.qty += m.qty;
      buckets.set(key, bucket);
    }
    const rows = [...buckets.values()].sort((a, b) => a.product.localeCompare(b.product) || a.reason.localeCompare(b.reason));

    const variance = reconciliations.map((r) => ({
      product: r.product.name, periodEnd: r.periodEnd, book: r.book, measured: r.measured, variance: r.variance, variancePct: r.variancePct, status: r.status,
    }));

    return NextResponse.json({ success: true, data: { rows, variance } });
  } catch (e) {
