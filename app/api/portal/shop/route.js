...
// Browse data for the self-service shop — the org's 'shop' (General Store) service, its active
// branches, and its priced/active products. No credit-limit info here (that's app/api/portal/me) —
// the actual check happens server-side at order time (lib/sale.js createPendingOrder).
export const GET = withOrg(async () => {
  try {
    const service = await prisma.service.findFirst({ where: { type: 'shop', isActive: true } });
    if (!service) throw new ApiError('This organization has no General Store service enabled', 404);
