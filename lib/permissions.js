...
const PERMISSIONS = {
  owner: ['*'],
  manager: ['users.invite', 'branches.manage', 'services.manage', 'shifts.run', 'sales.record', 'customers.manage'],
  // Counter work — running a shift, ringing up a sale, taking a payment — isn't management, so any
  // staff member with access to the branch can do it, not just managers. Changing a credit limit or
  // hold status IS management (an auditable, "worth watching" action) and stays manager+ only.
