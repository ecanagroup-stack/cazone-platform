  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // A `customer`-role login is a staff-provisioned portal account (Customer.userId) — resolve
        // which Customer it belongs to now so portal routes can scope to it without a per-request
        // lookup by anything the client controls.
        let customerId = null;
        if (user.role === 'customer') {
          const customer = await runUnscoped(() => prisma.customer.findUnique({ where: { userId: user.id }, select: { id: true } }));
