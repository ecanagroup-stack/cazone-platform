...
export async function createSaleOrder({ session, branchId, customerId, paymentMethod, lines, overrideCredit, stockReason = 'sale', channel, transportFee = 0, onOrderCreated }) {
  ...
  const grandTotal = subtotal + Math.round(Number(transportFee) || 0);
  ...
  const order = await prisma.$transaction(async (tx) => {
    ...
    const created = await tx.order.create({
      data: {
        branchId, customerId, orderNumber, subtotal, transportFee: Math.round(Number(transportFee) || 0), grandTotal, paymentMethod, channel: effectiveChannel, createdBy: session.user.id,
