...
// than always trusting the catalog price) — omit it to resolve the price normally, unchanged for
// every existing caller. `line.stockQty` similarly lets the *billed* quantity (drives pricing/
// lineTotal) differ from the *actual* quantity physically supplied (drives the stock/allocation
// decrement, in applyLineStock below) — omit it and they're the same, again unchanged by default.
async function priceLines(lines) {
  ...
  for (const line of lines) {
    ...
    if (line.unitPrice != null) {
      ...
      ({ price, priceRuleId } = await resolvePrice(line.productId));
    }
    ...
    priced.push({ productId: line.productId, qty, unitPrice: price, lineTotal, priceRuleId, allocationId: line.allocationId || null, stockQty });
  }
...
export async function createSaleOrder({ session, branchId, customerId, paymentMethod, lines, overrideCredit, stockReason = 'sale', channel, transportFee = 0, discount = 0, onOrderCreated }) {
  ...
  const grandTotal = subtotal - roundedDiscount + Math.round(Number(transportFee) || 0);
  ...
  const order = await prisma.$transaction(async (tx) => {
    ...
    const created = await tx.order.create({
      data: {
        branchId, customerId, orderNumber, subtotal, discount: roundedDiscount, transportFee: Math.round(Number(transportFee) || 0), grandTotal, paymentMethod, channel: effectiveChannel, createdBy: session.user.id,
