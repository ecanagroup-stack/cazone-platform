...
const FROM = () => process.env.REPORT_EMAIL_FROM || 'Cazone GS&M <noreply@cazone.app>';
...
const OTP_PURPOSE_LABEL = { credit_override: 'overriding a customer credit limit', price_approval: 'deciding a price change', customer_adjustment: 'applying a customer surcharge or refund' };
...
// lib/passwordReset.js's delivery mechanism — sent to whatever email is on file for the account
// (staff User.email or the Customer's own email, since a portal login IS a User row — see
// app/api/admin/customers/[id]/portal-access) requesting the reset, never anyone else's.
export async function sendPasswordResetEmail({ to, name, link }) {
  ...
  const { error } = await resend.emails.send({
    ...
    text: `Hi ${name},\n\nSomeone (hopefully you) asked to reset the password on this account. Click the link below to choose a new one — it expires in 1 hour and only works once:\n\n${link}\n\nIf you didn't request this, you can ignore this email — your password won't change.`,
  });
  if (error) throw new ApiError(error.message || 'Failed to send password reset email', 502);
}
